...
export class Common extends StackLayout {
  ...
  public userAction(id) {
    let self = this;
    let index: any;
    index = self.reviews.filter(item => {
      return item.id == id;
    });
  ...
  public init() {
    ...
    if (this.showHeader === "false") this.showHeader = false;
    ...
    this.rep.id = "mainrep";
    let reviewsDateTo;
    if (this.dateHandler) reviewsDateTo = this.dateHandler;
    else reviewsDateTo = "reviewsDateTo";

    this.rep.itemTemplate = `
        <GridLayout  dataid="{{ id,id }}"  longPress="{{$parents['Repeater'].LongPress,$parents['Repeater'].LongPress}}" ${
          plugin
        } class="review" rows="auto" columns="auto,*">
        ...
        <StackLayout orientation="horizontal">
          ...
          <Label row="0" col="1" text="{{ ${
            reviewsDateTo
          }(datetime) }}" class="review-datetime" textWrap="true" />
  ...
  constructor() {
    ...
    resources["getratearray"] = rate => {
      let arrayofitems = [];
      for (let i = 0; i < 5; i++) {
        if (i < rate) arrayofitems.push(true);
        else arrayofitems.push(false);
      }
      return arrayofitems;
    };
